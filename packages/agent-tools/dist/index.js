...
var BaseAgentTaskRequestSchema = z2.object({
  threadId: z2.string().uuid(),
  userId: z2.string().min(1),
  agentType: z2.enum(["CONTENT_GENERATION_AGENT", "TEXT_ANALYSIS_AGENT"]),
  taskSequence: z2.number().int().min(1),
  previousCSI: z2.object({
    completedSteps: z2.array(z2.string()).default([]),
    currentProgress: z2.number().default(0),
    totalSteps: z2.number().default(4),
    currentStep: z2.string().default("pending")
  }).optional()
});
...
var CoordinationRequestSchema = z3.object({
  threadId: z3.string().uuid(),
  userId: z3.string().uuid(),
  agentType: z3.enum(["CONTENT_GENERATION_AGENT", "TEXT_ANALYSIS_AGENT"]),
  operation: z3.string().min(1),
  lockOptions: z3.object({
    ttlSeconds: z3.number().int().positive().optional(),
    keyPrefix: z3.string().optional()
  }).optional(),
