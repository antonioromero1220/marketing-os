...
__export(src_exports, {
  ...
  BrandKitValidator: () => BrandKitValidator,
  BrandQualityAnalyzer: () => BrandQualityAnalyzer,
  ColorValidationSchema: () => ColorValidationSchema,
  DEFAULT_BRAND_KIT_DATA: () => DEFAULT_BRAND_KIT_DATA,
  FileUploadSchema: () => FileUploadSchema,
  FileValidationUtils: () => FileValidationUtils,
  FontValidationSchema: () => FontValidationSchema,
...
var FileUploadSchema = import_zod2.z.object({
  file: import_zod2.z.instanceof(File),
  assetType: import_zod2.z.enum(["logo", "product_photo", "lifestyle_image", "icon", "banner"]),
  title: import_zod2.z.string().optional(),
  description: import_zod2.z.string().optional()
});
var AssetMetadataSchema = import_zod2.z.object({
  file_name: import_zod2.z.string(),
  file_size: import_zod2.z.number().int().positive(),
  file_type: import_zod2.z.string(),
  uploaded_at: import_zod2.z.string().datetime(),
  width: import_zod2.z.number().int().positive().optional(),
  height: import_zod2.z.number().int().positive().optional(),
  alt_text: import_zod2.z.string().optional(),
  title: import_zod2.z.string().optional(),
  description: import_zod2.z.string().optional(),
  tags: import_zod2.z.array(import_zod2.z.string()).default([])
});
var UploadResultSchema = import_zod2.z.object({
  success: import_zod2.z.boolean(),
  assetUrl: import_zod2.z.string().url().optional(),
  storagePath: import_zod2.z.string().optional(),
  assetId: import_zod2.z.string().uuid().optional(),
...
var ColorValidationSchema = import_zod3.z.object({
  primary: import_zod3.z.string().regex(/^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$/, "Invalid hex color format"),
  secondary: import_zod3.z.string().regex(/^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$/, "Invalid hex color format").optional(),
  accent: import_zod3.z.string().regex(/^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$/, "Invalid hex color format").optional(),
  neutral: import_zod3.z.string().regex(/^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$/, "Invalid hex color format").optional()
});
var FontValidationSchema = import_zod3.z.object({
  heading: import_zod3.z.string().min(1, "Heading font is required").optional(),
  body: import_zod3.z.string().min(1, "Body font is required").optional(),
  accent: import_zod3.z.string().min(1, "Accent font name is required").optional()
});
